            ...
            for j in data.columns:
                ...
                if j == "时间":
                    ...
                    if "am" in element:
                        ...
                        # 格式化的字符串转换成Datetime
                        dt = datetime.datetime.strptime(element, "%d %m %Y, %H:%M")
                        date_time = str(dt)
                        print("时间：", date_time)
                        # 转化成时间戳
                ...
                if j == "content_time":
                    ...
                    print("内容：", content)
                    print("时间：", date_time)
                    timeArray = time.strptime(str(date_time), "%Y-%m-%d %H:%M:%S")
                    # 2将"2011-09-28 10:00:00"转化为时间戳
                    timestamp = int(time.mktime(timeArray))
                    print("timestamp:", timestamp)
                    time_stamp = timestamp
...
# ---------------------------------------------------------------------------------------
    ...
    """
    正则表达式，匹配“Wednesday 3 May 2017, 4:55pm”中的后面
    """
    @classmethod
    def match_timestamp(cls, timestamp):
        matchObj = re.search(r'(\d{1,2} \w{3} \d{4},.+)', timestamp, re.M | re.I)
        if matchObj:
...
# ---------------------------------------------------------------------------------------
    ...
    def format_datetime(cls, element):
        ...
        # dt = datetime.datetime.strptime(element, "%d %m %y")
        # date_time = str(dt)
        # return date_time
...
def main_operation():
    """Part1: 初始化参数"""
    file_path = '../../data/音乐服务2.csv'  # 读取文件路径和文件名
