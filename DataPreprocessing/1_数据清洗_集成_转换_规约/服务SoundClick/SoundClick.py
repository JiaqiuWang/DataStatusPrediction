...
# ---------------------------------------------------------------------------------------
    ...
    def match_timestamp(cls, timestamp):
        matchObj = re.search(r'(\d{1,2} \d{1,2}.*)', timestamp, re.M | re.I)
        if matchObj:
...
# ----------------------------------------------------------------------------------------------------------
    ...
    # 批量插入数据
    def format_datetime(self, element):
        ...
        print("completed match:", element)

        # 格式化的字符串转换成Datetime, 具体看字符串的格式 "%d %m %Y %H:%M:%S"
        dt = datetime.datetime.strptime(element, "%m %d_ %Y")
        date_time = str(dt)
        return date_time
...
def main_operation():
    """Part1: 初始化参数"""
    file_path = '../../data/音乐点击.csv'  # 读取文件路径和文件名
